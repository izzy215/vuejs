<script setup>
  ...
  // ref()로 정의한count 는 .value를 사용해 접근
  //reactive() 로 정의한 state는 .value를 사용하지 않고 접근
  const numberSum = computed(() => count.value + state.count);
  //함수 안에서도 ref() 로 정의한 데이터는 .value를 사용해 접근
  const expressFunc = function () {
    count.value +=1;
  };
  //함수 인에서도 reactive()로 정의한 데이터는 .value를 사용하지 않고 접근
  const arrowFunc = () => {
    state.count += 1;
  }
  </script>
