<script>
import OpteionsApi from '@/components/OptionsApi.vue';
import CompositionApi from '@/components/CompositionApi.vue';
import DataEx1 from '@/components/DataEx1.vue';
import DataEx2 from '@/components/DataEx2.vue';
import DataEx3 from '@/components/DataEx3.vue';
import RefEx1 from '@/components/RefEx1.vue';
import RefEx2 from '@/components/RefEx2.vue';
import ComputedEx1 from '@/components/ComputedEx1.vue';
import ComputedEx3 from '@/components/ComputedEx3.vue';
import ComputedEx4 from '@/components/ComputedEx4.vue';
import MethodsEx1 from '@/components/MethodsEx1.vue';
import WatchEx1 from '@/components/WatchEx1.vue';
import WatchEx2 from '@/components/WatchEx2.vue';
import WatchEx3 from '@/components/WatchEx3.vue';
import WatchEx4 from '@/components/WatchEx4.vue';
import WatchEx5 from '@/components/WatchEx5.vue';
import WatchEffectEx1 from '@/components/WatchEffectEx1.vue';
import WatchPostEffectEx1 from '@/components/WatchPostEffectEx1.vue';

import LifeCycleHooks from '@/components/LifeCycleHooks.vue';
import { ref } from 'vue';
const showComponent = ref(true);
const toggleComponent = () => {
  showComponent.value = !showComponent.value;
}

//export default {
  // components: {
  //   //OpteionsApi,
  //   //CompositionApi,
  //   // DataEx1,
  //   // DataEx2,
  //  // DataEx3,
  //   //RefEx1,
  //   // RefEx2,
  //   // ComputedEx1,
  //   // ComputedEx3,
  //   // ComputedEx4,
  //   // MethodsEx1,
  //   // WatchEx1,
  //   // WatchEx2,
  //   // WatchEx3,
  //   // WatchEx4,
  //   // WatchEx5,
  //   // WatchEffectEx1,
  //   // WatchPostEffectEx1

  // },
//}
</script>

<template>
  <!-- <OpteionsApi></OpteionsApi> -->
  <!-- <CompositionApi></CompositionApi> -->
   <!-- <DataEx1></DataEx1> -->
   <!-- <DataEx2></DataEx2> -->
   <!-- <DataEx3></DataEx3> -->
   <!-- <RefEx1></RefEx1> -->
   <!-- <RefEx2></RefEx2> -->
    <!-- <ComputedEx1></ComputedEx1> -->
    <!-- <ComputedEx3></ComputedEx3> -->
    <!-- <ComputedEx4></ComputedEx4> -->
     <!-- <MethodsEx1></MethodsEx1> -->
     <!-- <WatchEx1></WatchEx1> -->
     <!-- <WatchEx2></WatchEx2> -->
     <!-- <WatchEx3></WatchEx3> -->
     <!-- <WatchEx4></WatchEx4> -->
     <!-- <WatchEx5></WatchEx5> -->
     <!-- <WatchEffectEx1></WatchEffectEx1> -->
     <!-- <WatchPostEffectEx1></WatchPostEffectEx1> -->

     <LifeCycleHooks v-if="showComponent" />
     <button @click="toggleComponent">Unmount Component</button>

</template>

<style>
</style>
